...
1. The `mutate()` call creates a new column, strategy_type, using `case_when()` to assign a label based on each firm’s financial metrics. 

2. Case-wise Classification:
    * Case 1 – "High margin": Assigned if both conditions are true:
    ...
    * Case 2 – Missing data: If either profit_margin or asset_turnover is missing, the result is set to `NA_character_`
    * Case 3: Default case
