...
For purely mechanical reasons, outliers are an issue for a variable such as profit margin. Because the denominator (sales) can be small and the numerator (profit) can be negative, profit margin can easily 'blow up'. When sales are tiny, a moderate loss leads to a very large negative ratio. Thexe values create long left tails, making the histogram compress most observations into a tiny vertical slither at the right tail.
